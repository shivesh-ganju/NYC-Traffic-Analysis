...
year_green = [i for i in range(2014,2020)]
...
for year in year_fhv:
	...
	if year!=2019:
		...
		for i in range(1,13):
			url=""
			if i <10 :
				url = "https://nyc-tlc.s3.amazonaws.com/trip+data/fhvhv_tripdata_"+str(year)"-0"+str(i)+".csv"
			else:
				url = "https://nyc-tlc.s3.amazonaws.com/trip+data/fhvhv_tripdata_"+str(year)"-"+str(i)+".csv"
			file = urllib2.urlopen(url)
			writer = file.read()
			path = "/home/sg6148/"+str(folder_fhv)+str(year)+"_"+str(i)+".csv"
			with open(path,'wa') as a:
